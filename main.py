...
app.config['SQLALCHEMY_DATABASE_URI'] = os.environ.get('DB_URI','sqlite:///website_database.db')
...
def programs():
    ...
    return flask.render_template('programs.html', program=add_program, programs=program_projects,comment=comment_form)
...
def papers():
    ...
    if paper_form.validate_on_submit():
        ...
        paper_form.file.data.save(f"static/assets/thumbnail_pic/{paper_form.thumbnail.data.filename}")
        paper = Papers(
            ...
            thumbnail= paper_form.thumbnail.data.filename
    ...
    if add_art.validate_on_submit():
        ...
        if index <= 0:
            ...
            art = Art(
                ...
                date=datetime.date.today(),
                description=add_art.description.data
            )
            db.session.add(art)
            db.session.commit()
            return redirect(url_for('home'))
